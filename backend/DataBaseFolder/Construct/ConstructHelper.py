...
def DataBaseConstruct_Dish(ResNum, DishPerRes):
    ...
    for i in range(ResNum):
        for j in range(DishPerRes):
            TpDishInfo = rm.PyAddDish(i + 1, Method[int(j / 4) % 4] + Object[j % 5], random.randint(10, 100),
                                      random.randint(1, 3), j % 5, 'pic', 'des')
...
def RandomDishPicture(index):
    ...
    '''
    PicUrl = [
        'https://gimg2.baidu.com/image_search/src=http%3A%2F%2Fi2.chuimg.com%2F74ff11e14fa64672b79e14bc94916cdf_960w_1200h.jpg%3FimageView2%2F2%2Fw%2F600%2Finterlace%2F1%2Fq%2F90&refer=http%3A%2F%2Fi2.chuimg.com&app=2002&size=f9999,10000&q=a80&n=0&g=0n&fmt=jpeg?sec=1632707496&t=9013abdaa584b8a872725d55f1cb250b',
...
def DataBaseConstruct_Order(UserNum, DishPerRes, OrderPerUser, MaxOrderLen, OrderDateLen):
    ...
    for i in range(UserNum):
        for j in range(OrderPerUser):
            ...
            orderinfo = rm.PyAddOrder(1, random.randint(1, UserNum), '', 'Home',
                                      RandomDishesInOrder(DishPerRes, MaxOrderLen), price, random.randint(0, 20))
