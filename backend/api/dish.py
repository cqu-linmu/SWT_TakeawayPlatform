...
# 添加餐品
@route_dish.route("/add", methods=["GET"])
def addDish():
    ...
    @route_dish.route("/edit", methods=['POST'])
    def editDish():
        ...
        # 修改数据中菜品的信息
        dish_id = req['dish_id'] if 'dish_id' in req else ''  # 菜品编号
        dish_class = int(req['dish_class']) if 'dish_class' in req else ''  # 菜品分类
        dish_name = req['dish_name'] if 'dish_name' in req else ''  # 菜品名
        dish_price = int(req['dish_price']) if 'dish_price' in req else ''  # 菜品价格
        dish_description = req['dish_description'] if 'dish_description' in req else ''  # 菜品描述
        ...
        if dish_description is None or len(dish_description) < 3:
            ...
            resp['message'] = "请输入描述，并不能少于10个字符"
            return jsonify(resp)
        if not GenericModify(3, dish_id, 'Dish', ['DishName', 'Price', 'DishType', 'Description'], ['\'dish_name\'', \
                                                                                           dish_price,
                                                                                           dish_class,
                                                                                           '\'dish_description\'', ]):
            resp['code'] = 400
